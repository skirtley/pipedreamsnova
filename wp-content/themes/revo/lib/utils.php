...
add_action( 'save_post', 'revo_product_save_meta', 10, 1 );
function post_init(){
	add_meta_box("revo_product_meta", esc_html__( 'Product Meta:', 'revo' ), "revo_product_meta", "product", "side", "low");
	add_meta_box("revo_product_video_meta", esc_html__( 'Featured Video Product', 'revo' ), "revo_product_video_meta", "product", "side", "low");
}	
function revo_product_meta(){
	global $post;
	$recommend_product = get_post_meta( $post->ID, 'recommend_product', true );
	$newproduct 	   = get_post_meta( $post->ID, 'newproduct', true );
?>
	<p><label><b><?php esc_html_e( 'Recommend Product:', 'revo' ) ?></b></label> &nbsp;&nbsp;
	<input type="checkbox" name="recommend_product" value="1" <?php echo checked( $recommend_product, 1 ) ?> /></p>
	
	<p><label><b><?php esc_html_e( 'New Product', 'revo' ) ?></b></label> &nbsp;&nbsp;
		<input type="number" name="newproduct" value="<?php echo esc_attr( $newproduct ) ?>"/>
		<span class="p-description"><?php echo esc_html__( 'Set day for the new product label from the date publish product.', 'revo' ); ?></span>
	</p>
<?php }

function revo_product_video_meta(){
	global $post;
...
?>
	<div class="featured-image">
		...
		<?php endif; ?>
		<p><input type="text" name="featured_video_product" placeholder="<?php echo esc_attr__( 'Youtube Video ID', 'revo' ) ?>" value="<?php echo esc_attr( $featured_video_product ); ?>"/></p>
	</div>
<?php 
}

function revo_product_save_meta( $post_id ){
